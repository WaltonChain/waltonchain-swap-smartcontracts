...
contract Repurchase is Ownable {
    ...
    constructor(
        ...
        require(_wst != address(0), "Is zero address");
        require(_wst_usdt != address(0), "Is zero address");
    ...
    function swap() external onlyCaller returns (uint256 amountOut) {
        ...
        (uint256 reserve0, uint256 reserve1, ) =
            IWswapPair(WST_USDT).getReserves();
        uint256 amountInWithFee = amountIn.mul(997);
        amountOut =
            amountIn.mul(997).mul(reserve0) /
            reserve1.mul(1000).add(amountInWithFee);
        IERC20(USDT).safeTransfer(WST_USDT, amountIn);
        IWswapPair(WST_USDT).swap(amountOut, 0, destroyAddress, new bytes(0));
    }
