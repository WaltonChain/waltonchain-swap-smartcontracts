...
contract SwapMining is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocWstAmount; // How many WSTs
    ...
    // Only tokens in the whitelist can be mined WST
    ...
    function getWstReward(uint256 _lastRewardBlock)
        public
        view
        returns (uint256)
    {
    ...
    function mint(uint256 _pid) public returns (bool) {
        ...
        uint256 blockReward = getWstReward(pool.lastRewardBlock);
        if (blockReward <= 0) {
            return false;
        }
        ...
        wst.mint(address(this), xtReward);
        // Increase the number of tokens in the current pool
        pool.allocWstAmount = pool.allocWstAmount.add(xtReward);
    ...
    function swap(
        ...
        address pair = IWswapFactory(factory).pairFor(input, output);
    ...
    function takerWithdraw() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            if (user.quantity > 0) {
                ...
                uint256 userReward =
                    pool.allocWstAmount.mul(user.quantity).div(pool.quantity);
                pool.quantity = pool.quantity.sub(user.quantity);
                pool.allocWstAmount = pool.allocWstAmount.sub(userReward);
                user.quantity = 0;
                user.blockNumber = block.number;
        ...
        wst.transfer(msg.sender, userSub);
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (user.quantity > 0) {
            ...
            uint256 xtReward =
                blockReward.mul(pool.allocPoint).div(totalAllocPoint);
            userSub = userSub.add(
                (pool.allocWstAmount.add(xtReward)).mul(user.quantity).div(
                    pool.quantity
                )
            );
        }
        //Wst available to users, User transaction amount
        ...
        )
    {
        require(_pid <= poolInfo.length - 1, "SwapMining: Not find this pool");
        PoolInfo memory pool = poolInfo[_pid];
        address token0 = IWswapPair(pool.pair).token0();
        address token1 = IWswapPair(pool.pair).token1();
        uint256 xtAmount = pool.allocWstAmount;
        uint256 blockReward = getWstReward(pool.lastRewardBlock);
        uint256 xtReward =
            blockReward.mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function getQuantity(
        ...
        if (outputToken == anchorToken) {
            ...
            IWswapFactory(factory).getPair(outputToken, anchorToken) !=
            address(0)
        ) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    IWswapFactory(factory).getPair(outputToken, intermediate) !=
                    address(0) &&
                    IWswapFactory(factory).getPair(intermediate, anchorToken) !=
                    address(0)
                ) {
